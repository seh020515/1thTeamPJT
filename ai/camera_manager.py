...
def mouse_callback(event, x, y, flags, param):
    ...
    global DANGER_X1, DANGER_Y1, DANGER_X2, DANGER_Y2
...
# ESP32-CAM 사용할 때는 위 줄 대신 아래처럼 사용
# camera = None
...
shark_model = YOLO(
    r"C:\seh\firstPjt\1thTeamPJT\1thTeamPjt\runs\detect\train-2\weights\best.pt"
)
...
def reconnect_camera():
    ...
    # ESP32-CAM 사용할 때는 위 줄 대신 아래 줄 사용
    # camera = cv2.VideoCapture(ESP32_STREAM_URL)
...
def get_frame():
    global camera
    global last_intrusion_time, last_shark_time, last_crowd_time
    ...
    try:
        ...
        for box in result.boxes:
            ...
            if (
                DANGER_X1 <= center_x <= DANGER_X2 and
        ...
        if is_intrusion:
            ...
            if (
                last_intrusion_time is None or
        ...
        if danger_count >= CROWD_LIMIT:
            cv2.putText(
                frame,
                f"CROWD WARNING: {danger_count} people",
                (30, 130),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 165, 255),
